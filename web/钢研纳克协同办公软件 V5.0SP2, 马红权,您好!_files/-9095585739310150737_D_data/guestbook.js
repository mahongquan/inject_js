...
{ 
	...
	if(iWidth>350)
	oReplyDiv.innerHTML="<div class='header'><div class='title'>"+_("MainLang.leaveMessageRange")+"</div><div class='close' onclick='hiddenLeaveWordDlg(\""+idstr+"\")'></div></div><div class='content'><textarea id='contentReplyArea"+idstr+"' class='contentArea'></textarea></div><div class='footer'><div class='sentMessage'><label for='sendMessage'><input id='sendMessage"+idstr+"' checked type='checkbox'/>"+_("MainLang.sendMessage")+"</label></div><div class='sentButton'><input type='button' class='sendAction' id='sendActionButton"+idstr+"' onclick=\"sendMessage('"+departmentId+"','"+replyId+"','"+replyerId+"','"+idstr+"')\" value='"+_("MainLang.sendAction")+"'/></div></div>";
	else{
		oReplyDiv.innerHTML="<div class='header'><div class='title'>"+_("MainLang.leaveMessageRange")+"</div><div class='close' onclick='hiddenLeaveWordDlg(\""+idstr+"\")'></div></div><div class='content'><textarea id='contentReplyArea"+idstr+"' style='width: "+(iWidth-1)+"px;height: 80px;border: 1px #AFAEA8 solid'></textarea></div><div class='footer'><div class='sentMessage'><label for='sendMessage'><input id='sendMessage"+idstr+"' checked type='checkbox'/>"+_("MainLang.sendMessage")+"</label></div><div class='sentButton'><input type='button' class='sendAction' id='sendActionButton"+idstr+"' onclick=\"sendMessage('"+departmentId+"','"+replyId+"','"+replyerId+"','"+idstr+"')\" value='"+_("MainLang.sendAction")+"'/></div></div>";
	}
 ...
 * 公示板发表内容
 */
function showNoticeDiv(idstr, spaceType, spaceId, fragmentId, ordinal, boardId){
	var oReplyDiv = document.getElementById('replyDiv' + idstr);
	var oShowLeaveWordsDIV = document.getElementById(idstr);
	var iWidth = parseInt(oShowLeaveWordsDIV.clientWidth);
	if(iWidth > 350){
	...
	oReplyDiv.innerHTML = 
		...
		"</div>" +
		"<div class='content'><textarea id='contentReplyArea" + idstr + "' class='contentArea'>" + contentReplyAreaHtml + "</textarea></div>" +
		"<div class='footer'>" +
		...
		"<div class='sentButton'><input type='button' class='sendAction' id='sendActionButton" + idstr + "' onclick='sendNotice(\"" + idstr + "\", \"" + spaceType + "\", \"" + spaceId + "\", \"" + fragmentId + "\", \"" + ordinal + "\", \"" + boardId + "\")' value='" + _("MainLang.sendAction") + "'/></div>" +
		"</div>";
	}else{
		oReplyDiv.innerHTML = 
			"<div class='header'>" +
			...
			"<div class='close' onclick='hiddenLeaveWordDlg(\"" + idstr + "\")'></div>" +
			"</div>" +
			"<div class='content'><textarea id='contentReplyArea" + idstr+"' style='width: "+(iWidth-1)+"px;height: 80px;border: 1px #AFAEA8 solid'>" + contentReplyAreaHtml + "</textarea></div>" +
			"<div class='footer'>" +
			"<div class='sentMessage'><label for='sendMessage" + idstr + "'><input id='sendMessage" + idstr + "' type='checkbox'/>" + _("MainLang.sendMessage") + "</label></div>" +
			"<div class='sentButton'><input type='button' class='sendAction' id='sendActionButton" + idstr + "' onclick='sendNotice(\"" + idstr + "\", \"" + spaceType + "\", \"" + spaceId + "\", \"" + fragmentId + "\", \"" + ordinal + "\", \"" + boardId + "\")' value='" + _("MainLang.sendAction") + "'/></div>" +
			"</div>";	
 ...
 * 保存公示板内容
 */
function sendNotice(idstr, spaceType, spaceId, fragmentId, ordinal, boardId){
	try{
		var sendMessage = 'true';
		var oSendMessage = document.getElementById('sendMessage' + idstr);
		...
		if(oContentReplyArea){
			...
			if(contentReplyMessage != ''){
				if(contentReplyMessage.length > 600){
					alert(_("MainLang.notice_content_length", 600));
					return;
				}
			}else{
