...
jQuery.fn.getMenu = function(cut){
    ...
    $(data).each(function(i){
      ...
      menuObj += "<a onclick='showShortcut(\""+data[i].urlKey.escapeUrl()+"\")' id='" + data[i].idKey + "' class='hand'>" + data[i].nameKey + "</a>";
        //});
       // menuObj += "</div>";
    });
    menuObj += "</div>";
...
function refreshAccountName(accountName,secondName){
  new portalTemplateManager().getTemplateHospot($.ctx.template[0].id,{
